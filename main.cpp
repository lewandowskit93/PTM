    ...
    MainApp() :
        Application(), _screen_timer(
            ManagedTimer(&_timer_manager, 40,
                std::bind(&MainApp::onScreenUpdate, this), true)), _canvas(84,
            48), _display(
            ...
            0, 84, 48), _ptero(10,10)
    {
      ...
      _event_listener.registerEventHandler(
          EventMapping(EVENT_BUTTON,
              std::bind(&MainApp::handleButton, this, std::placeholders::_1)));
    }
...
int main(void)
{
  ...
  std::weak_ptr<Accelometer> acc =
      System::getInstance()->_device_manager.mountDevice<Accelometer>(SPI1,
          ...
          Pin(GPIOE, GPIO_Pin_3), Pin(GPIOE, GPIO_Pin_0),
